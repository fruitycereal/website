...
if ($role == 'member') {
    ...
    if (mysqli_num_rows($turn_in_result) > 0) {
        ...
        $file_path = $turn_in_data['file_path']; 
    }
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['turn_in']) && !$has_turned_in && $role == 'member') {
    ...
    $turn_in_sql = "INSERT INTO turn_ins (project_id, user_id, status, file_path) VALUES ('$project_id', '$user_id', '$status', '$file_path')";
    ...
    <div class="main-content">
        ...
        <?php if ($role == 'member') { ?>
            <?php if (!$has_turned_in) { ?>
                <form method="POST" action="view_project.php?project_id=<?php echo $project_id; ?>" enctype="multipart/form-data">
                    <label for="file_upload">Attach your work (optional):</label>
                    <input type="file" name="file_upload" id="file_upload">
                    <button type="submit" name="turn_in">Turn in Project</button>
                </form>
            <?php } else { ?>
                <p>You have already turned in this project</p>
                <p><strong>Status:</strong> <?php echo htmlspecialchars($status); ?></p>
                <p><strong>Attached File:</strong> <a href="<?php echo htmlspecialchars($file_path); ?>" target="_blank">Download</a></p>
                <form method="POST" action="view_project.php?project_id=<?php echo $project_id; ?>">
                    <button type="submit" name="unturn_in">Unturn Project</button>
                </form>
            <?php } ?>
        <?php } else { ?>
